...
void lsfile(char *path, int level, int fd1)
{
	char buf[512], *p;
	int fd;
	char a[4] = "|--";
...
void ls(char *path, int level,int dest_level, int mode)
{
	if (level+1>dest_level && dest_level != 0)
	{
		return;
	} else
	{
			...
			*p++ = '/';
			while (read(fd, &de, sizeof(de)) == sizeof(de))
			{
				...
				if (st.type == T_DIR)
				{
					...
					char *name = fmtname(buf);
					if (name[0] != '.')
						ls(buf, level + 1,dest_level, mode);
				}
				else if (mode != 1)
				{
					if (level == 0)
	...
	else
	{
		if (argv[1][0] == '-')
		{
			...
			else if (argv[1][1] == 'L')
			{
				if (argc < 3)
				{
					printf(1, "error: usage tree -L <depth> <path>\n");
				}
				else
				{
					...
					if (depth == 0)
					{
						printf(1,"tree: invalid level, must be greater than 0\n");
					}
					else if (argc>3)
					{
						int i;
